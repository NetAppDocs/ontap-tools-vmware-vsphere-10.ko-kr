...
[role="lead"]
기본적으로 ESXi 호스트에서 검색된 모든 IP 주소는 네트워크 액세스를 구성하지 않는 한 자동으로 내보내기 정책에 추가됩니다.  특정 IP 주소에서만 액세스를 허용하도록 내보내기 정책을 수정할 수 있습니다.  제외된 ESXi 호스트가 마운트 작업을 시도하면 작업이 실패합니다.
