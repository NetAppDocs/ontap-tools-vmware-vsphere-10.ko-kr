...
ONTAP tools Manager에 vCenter Server 인스턴스를 등록할 때 vCenter Server 인증서에 유효한 SAN(Subject Alternative Name) 확장이 포함되어 있는지 확인하십시오. SAN 확장에는 DNS 및 IP 주소 항목이 모두 포함되어야 합니다. 예를 들면 다음과 같습니다.
...
----
vCenter Server 인증서에 SAN 확장이 포함되어 있지 않거나 SAN 확장에 올바른 DNS 또는 IP 주소 값이 포함되어 있지 않은 경우, 인증서 유효성 검사 오류로 인해 ONTAP tools 작업이 실패할 수 있습니다.
