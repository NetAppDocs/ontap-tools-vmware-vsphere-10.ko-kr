...
. *Storage* 창에서 스토리지 VM을 선택합니다. 필요에 따라 *Advanced options* 섹션에서 *Custom initiator group name*을 입력합니다. 데이터스토어에 기존 igroup을 선택하거나 사용자 지정 이름으로 새 igroup을 생성할 수 있습니다.
+
