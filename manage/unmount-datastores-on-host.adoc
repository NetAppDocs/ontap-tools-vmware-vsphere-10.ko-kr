...
---
= ONTAP 도구에서 NFS 및 VMFS 데이터 저장소 마운트 해제
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
데이터 저장소 마운트 해제 작업은 ESXi 호스트에서 NFS 또는 VMFS 데이터 저장소를 제거합니다.  ONTAP tools for VMware vSphere 에서 검색하거나 관리하는 데이터 저장소에 사용할 수 있습니다.

.단계
. vSphere Client에 로그인합니다.
. NFS 또는 VMFS 데이터 저장소 객체를 마우스 오른쪽 버튼으로 클릭하고 *데이터 저장소 마운트 해제*를 선택합니다.
+
vSphere 클라이언트는 대화 상자를 열고 데이터스토어를 마운트하는 ESXi 호스트를 나열합니다. 보호된 데이터 저장소에서 작업을 수행하면 화면에 경고 메시지가 표시됩니다.
