...
summary:  
---
= ONTAP tools가 igroup 및 엑스포트 정책을 관리하는 방법
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
ONTAP tools 10.5P2부터 마이그레이션된 igroup은 더 이상 사용되지 않으면 삭제됩니다. 이전 버전에서는 마이그레이션된 igroup이 제거되지 않았습니다. 다음 사항에 유의하십시오.

* 마이그레이션된 igroup은 데이터 저장소 간에 재사용할 수 없습니다.
* 사용자 정의 igroup은 삭제되지 않으며 계속 재사용할 수 있습니다.
