...
가상 머신을 다른 FlexVol volume 으로 마이그레이션하는 경우 시스템은 해당 볼륨의 메타데이터 파일을 가상 머신 정보로 업데이트합니다. 가상 머신이 동일한 스토리지를 사용하지만 다른 ESXi 호스트로 마이그레이션되는 경우 기본 FlexVol volume 메타데이터 파일은 수정되지 않습니다.
...
메모리 정보가 포함되지 않은 가상 머신 스냅샷이 지원됩니다. 메모리 정보가 포함된 스냅샷이 있는 가상 머신은 보호할 수 없습니다.

메모리 스냅샷이 포함된 보호되지 않은 가상 머신은 보호할 수 없습니다. 가상 머신에 대한 보호를 활성화하기 전에 메모리 스냅샷을 삭제하십시오.

ASA r2 스토리지 유형을 사용하는 Windows VM의 경우 가상 머신 스냅샷은 읽기 전용입니다. VM을 켜면 VASA Provider가 읽기 전용 스냅샷에서 LUN을 생성하고 IOPS를 활성화합니다. VM을 끄면 VASA Provider가 LUN을 삭제하고 IOPS를 비활성화합니다.
