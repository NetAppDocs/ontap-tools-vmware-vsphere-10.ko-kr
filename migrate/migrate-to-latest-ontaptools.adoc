...
.. 줄임표 메뉴를 클릭하고 * Reset configuration * 을 선택합니다
...
VMware vSphere용 ONTAP 툴의 이전 릴리스에서 10.3으로 마이그레이션한 후 SRA 어댑터를 다시 검색하여 VMware 라이브 사이트 복구 스토리지 복제 어댑터 페이지에 세부 정보가 업데이트되는지 확인합니다.
