...
.단계
. URL `\https://<srm_ip>:5480`을 사용하여 VMware Live Site Recovery 어플라이언스 관리 인터페이스에 로그인한 다음 VMware Live Site Recovery 어플라이언스 관리 인터페이스에서 Storage Replication Adapters로 이동합니다.
. 새 어댑터 * 를 선택합니다.
