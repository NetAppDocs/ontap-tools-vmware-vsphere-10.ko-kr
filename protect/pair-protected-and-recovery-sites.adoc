...
NOTE: SRA(Storage Replication Adapter)는 팬아웃 SnapMirror 구성을 지원하지 않습니다. SnapMirror 팬아웃 구성은 소스 볼륨이 서로 다른 두 대상에 복제되는 구성입니다. SRM이 대상에서 가상 시스템을 복구해야 하는 경우 복구 중에 문제가 발생합니다.

* 필요한 것 *

* 보호 사이트와 복구 사이트에 SRM(Site Recovery Manager)이 설치되어 있어야 합니다.
