...
| 084 | JobManager를 프로덕션으로 다시 변경하지 못했습니다 | 업그레이드를 복구/완료하려면 다음 단계를 수행하십시오. 1. Diagnostic Shell 2를 활성화합니다. 다음 명령을 실행합니다. _sudo perl /home/maint/scripts/post-deploy-upgrade.pl --postupgrade_ 3. /var/log/post-deploy-upgrade.log에서 로그를 확인합니다. 
...
| 088 | 저널러에 대한 로그 회전을 구성하지 못했습니다 | VM이 호스팅되는 호스트와 호환되는 VM 네트워크 설정을 확인하십시오. VM을 다른 호스트로 마이그레이션한 후 다시 시작해 볼 수 있습니다. 
...
| 104 | 보조 노드를 다시 시작하지 못했습니다 | 노드를 하나씩 수동으로 재시작합니다. 
...
업그레이드가 오류 코드 114와 함께 실패하는 경우 다음 단계를 수행하십시오.

. 운영 노드에서 업그레이드 ISO에 연결하고 진단 셸을 활성화하십시오.
. SSH 세션에서 운영 노드에 ISO를 마운트합니다.
+
[listing]
----
...
----
. 진단 셸을 활성화하고 다음을 실행하여 컬렉션에 인증서를 추가합니다.
+
[listing]
----
...
----
+
 `/var/log/otvng/post-deploy-upgrade.log`에서 로그를 확인하십시오.
...
----
+
성공 메시지를 찾습니다. `Completed post 10.5 upgrade steps.`
