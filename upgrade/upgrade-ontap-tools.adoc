...
[role="lead"]
HA 및 비 HA 구현 모두에서 업그레이드가 지원됩니다. 지원되는 업그레이드 경로는 다음과 같습니다.
...
NOTE: VM ware vSphere 10.1 및 10.2에서 10.3까지의 ONTAP 툴에서 업그레이드가 지원됩니다. ONTAP 도구 10.0에서 10.3으로 직접 업그레이드할 수 없습니다.

* 시작하기 전에 *

HA가 아닌 업그레이드의 경우 ONTAP 도구 VM의 전원을 끄고 HA 업그레이드의 경우 가상 머신(VM) 설정을 다음과 같이 변경하기 전에 첫 번째 노드의 전원을 끕니다.

* 서비스 데이터가 VM에 로컬로 저장될 때 각 노드에 100GB의 추가 하드 디스크를 추가합니다.
* 배포 방식에 따라 전원이 꺼진 VM의 CPU 및 메모리를 변경합니다. CPU 및 RAM에 대해 핫 플러그인을 활성화합니다.
+
|===
...
* HA 배포의 경우 리소스를 변경하고, CPU 및 RAM에 대한 핫 플러그인을 활성화하고, 두 번째 노드와 세 번째 노드에 대해 100GB 하드 디스크를 추가합니다. 이러한 노드를 재부팅할 필요가 없습니다.
* 어플라이언스가 ONTAP tools 10.1 또는 10.2 버전을 사용하여 로컬 경로(간편한 배포)로 배포된 경우 업그레이드하기 전에 스냅샷을 중지해야 합니다.
...
. 터미널/putty에서 이전 단계에서 설정한 'DIAG' 및 암호로 VM IP 주소에 로그인합니다.
...
* 스냅샷 탭으로 이동하고 * 스냅샷 촬영 * 버튼을 클릭합니다. VM의 일시 중지된 스냅샷을 생성합니다. 자세한 내용은 을 https://docs.vmware.com/en/VMware-vSphere/7.0/com.vmware.vsphere.vm_admin.doc/GUID-9720B104-9875-4C2C-A878-F1C351A4F3D8.html["가상 시스템의 스냅샷을 생성합니다"^] 참조하십시오.


업그레이드를 수행하기 전에 로그 번들에서 접두사 "generate-support-bundle-job"이 있는 완료된 Pod를 삭제하십시오.
지원 번들 생성이 진행 중인 경우 완료될 때까지 기다린 다음 Pod를 삭제하십시오.

모든 유형의 업그레이드를 위해 100GB 크기의 하드 디스크 드라이브(HDD)를 추가해야 합니다. HDD를 추가하려면 다음 작업을 수행하십시오.

. 단일 노드 구성의 VM을 선택하거나 HA 구성의 VM 3개를 모두 선택합니다.
. VM을 마우스 오른쪽 버튼으로 클릭하고 * Add New Device * > * Hard Disk * 를 선택합니다
. 새 하드 디스크 * 필드에 100GB 크기의 HDD를 추가합니다.
. 적용 * 을 클릭합니다


하드 디스크를 추가한 후 해당 구성에 대한 VM의 리소스를 업데이트하고 기본 VM을 다시 시작합니다.
...
* HA가 아닌 소규모 구성을 HA가 아닌 미디어 또는 HA 중간 규모 또는 대규모 구성으로 확장합니다.
* HA가 아닌 업그레이드의 경우 변경 사항을 반영하도록 ONTAP 도구 VM을 재부팅합니다. HA를 업그레이드하는 경우 첫 번째 노드를 재부팅하여 노드의 변경 사항을 반영합니다.


* 완료 후 *

VMware vSphere용 ONTAP 툴의 이전 릴리스에서 10.3 릴리스로 업그레이드한 후에는 SRA 어댑터를 다시 검색하여 VMware 라이브 사이트 복구 스토리지 복제 어댑터 페이지에서 세부 정보가 업데이트되는지 확인합니다.

성공적으로 업그레이드한 후 다음 절차에 따라 ONTAP에서 Trident 볼륨을 수동으로 삭제합니다.


NOTE: VMware vSphere 10.1 또는 10.2용 ONTAP 툴이 HA가 아닌 소규모 또는 비 HA 중간 규모(로컬 경로) 구성인 경우에는 이러한 단계가 필요하지 않습니다.
...
이렇게 하면 VMware vSphere 10.1/10.2용 ONTAP 툴에 사용된 ONTAP에 있는 Trident 볼륨이 모두 삭제됩니다.
