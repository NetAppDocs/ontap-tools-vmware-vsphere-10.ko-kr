...
+
두 개의 vCenter Server 토폴로지에서 ONTAP 도구를 배포하는 경우(어플라이언스가 한 vCenter 인스턴스에 호스팅되고 다른 vCenter 인스턴스를 관리하는 경우) ONTAP 도구를 호스팅하는 vCenter 인스턴스에 제한된 역할을 할당할 수 있습니다.  OVF 템플릿 배포에 필요한 아래 나열된 데이터 저장소 권한만 사용하여 전담 vCenter 사용자 및 역할을 만들 수 있습니다.
