...
. URL에서 OVA 파일을 선택하거나 해당 위치를 찾은 후 * 다음 * 을 선택합니다.
...
. Select a name and folder * 필드에 가상 머신 이름을 입력하고 위치를 선택합니다. * 이 가상 시스템의 하드웨어 사용자 정의 * 확인란을 선택하면 * 완료 준비 * 창에 도달하기 전에 * 하드웨어 사용자 정의 * 라는 추가 단계가 활성화됩니다.
...
. 하드웨어 사용자 정의* 창에서 CPU 핫 추가 및 메모리 핫 플러그인을 활성화합니다.
. 작업 완료 후 VM의 전원을 켭니다.
